...
exports.currentTurn = async function () {
    let json = await store.currentTurn('currentTurn');
    if (json != null) {
    ...
    store.currentTurn('currentTurn', JSON.stringify(currentTurn));
    return new Mob(currentTurn);
};
exports.start = function (lengthInMinutes) {
    let mobTurn = new MobTurn(lengthInMinutes * 60);
    store.currentTurn('currentTurn', JSON.stringify(mobTurn));
    return new Mob(mobTurn);
};
